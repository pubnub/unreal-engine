...
void ASample_GroupsFull::RunGroupsFullExample()
{
	...
	UE_LOG(LogTemp, Log, TEXT("Groups example, User ID is set"));
	...
	UE_LOG(LogTemp, Log, TEXT("Groups example, subscribed to group: %s"), *ChannelGroup);
		
	// 3. Publish a message to the channel (which is in the subscribed group)
	...
	UE_LOG(LogTemp, Log, TEXT("Groups example, message published to channel: %s"), *Channel);
}
...
void ASample_GroupsFull::OnAddChannelToGroupResponse(FPubnubOperationResult Result)
{
	...
	{
		UE_LOG(LogTemp, Log, TEXT("Groups example, channel successfully added to group."));
	}
}

void ASample_GroupsFull::OnPubnubMessageReceived(FPubnubMessageData Message)
{
	UE_LOG(LogTemp, Log, TEXT("Groups example, message received on Channel: %s, via Group: %s, Message Content: %s"), *Message.Channel, *Message.MatchOrGroup, *Message.Message);
}
...
void ASample_GroupsFull::OnPublishResult(FPubnubOperationResult Result, FPubnubMessageData Message)
{
	if(Result.Error)
	{
		UE_LOG(LogTemp, Error, TEXT("Groups example, failed to publish message. Status: %d, Reason: %s"), Result.Status, *Result.ErrorMessage);
	}
	else
	{
		UE_LOG(LogTemp, Log, TEXT("Groups example, message published successfully. Published message timetoken: %s"), *Message.Timetoken);
	}
}

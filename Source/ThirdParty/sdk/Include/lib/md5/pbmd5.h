...
/** @file pbmd5.h 
    ...
    Here we defer to the "most often used" interface of MD5 module(s).
    We ignore the results, even if `MD5_` functions do return some
    (most don't).
*/
 ...
 */
#define pbmd5_init(x) MD5_InitPN(x)
 ...
 */
#define pbmd5_update(x, m, l) MD5_UpdatePN((x), (m), (l))
 ...
 */
#define pbmd5_update_str(x, str) MD5_UpdatePN((x), (str), strlen(str))
...
*/
#define pbmd5_final(x, d) MD5_FinalPN((d), (x))
...
#define pbmd5_digest_str(str, d) do { MD5_CTX M_ctx_;   \
        MD5_InitPN(&M_ctx_);                              \
        MD5_UpdatePN(&M_ctx_, (str), strlen(str));        \
        MD5_FinalPN((d), &M_ctx);                         \
    } while (0)
